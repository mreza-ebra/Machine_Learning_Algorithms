...
% Sigma=cov(X);
% for i=1:M
%    prob(i)=exp((-0.5*(X(:,i)-Mu)'/(Sigma))*(X(:,i)-Mu))/((2*pi)^(N/2)*sqrt(det(Sigma)));
% end

prob=diag(exp((-0.5*(X-Mu)'/(Sigma))*(X-Mu))/((2*pi)^(N/2)*sqrt(det(Sigma))))'; 
% Maybe this way is faster than a for loop!!!
